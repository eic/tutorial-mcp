...
Because MCP is a standard ([Episode 3](03-mcp-servers.md)), the collaboration can expose each piece
of its infrastructure as a small server that *any* assistant can use. This page is a working
catalogue of those servers. You connect each one exactly as you connected `uproot` — start it with
`eic-mcp up` and point your assistant at its SSE URL.
...
This is a snapshot (mid-2026). Most servers below are public and working; one (LXR) is planned;
one (indico) is currently maintained by an individual contributor rather than the `eic` org.
Always check the [eic GitHub organisation](https://github.com/eic) and the
...
statistics, histograms, and sandboxed NumPy/awkward kernels. This is the analysis backend you
drove in Episodes 3 and 5.
...
([docs](https://eic.github.io/xrootd-mcp-server/)) lets an assistant browse the JLab/dCache
XRootD store — list directories, read file metadata, search, and monitor production campaigns —
so it can locate the files an analysis needs.
...
[Rucio](https://rucio.cern.ch/) — the scientific data-management system the lesson uses to locate
datasets — through ~13 tools (dataset discovery, file listing, quotas, replication rules), with
X.509 / username authentication for the BNL and JLab instances.
...
and (when explicitly enabled) manage depositions and uploads — so an assistant can find and cite
archived data and software.
...
individual contributor and works with any Indico server.
...
code — definitions, references, call sites — the way you navigate a codebase in an editor. No
public standalone repository exists yet; it is referenced as a tool inside the corun-ai
...
orchestrates AI workflows for the collaboration — the BNL project mentioned in the workshop's
motivation.

* [**`BNLNPPS/corun-ai`**](https://github.com/BNLNPPS/corun-ai) — a collaborative AI-workflow harness:
  humans provide input and tool access, the AI processes asynchronously through defined pipelines,
  and results are curated. It is deployed at the [ePIC dev-cloud](https://epic-devcloud.org/doc/) to
  generate documentation for the EIC reconstruction software (the *codoc* portal).
* [**`eic/corun-mcp-server`**](https://github.com/eic/corun-mcp-server) — wraps the corun-ai REST API
  as an MCP server, so an assistant can browse documentation, submit prompts, trigger generation
  jobs, and poll results.
...
This is exactly the central infrastructure the workshop set out to discuss: shared MCP servers and
proxies, a catalogue of skills, and agent harnesses any collaborator can reuse. You now have the
full picture — from a single free assistant and one tool server, up to a collaboration-wide
ecosystem — and a real Λ⁰ measurement you carried out yourself along the way.
