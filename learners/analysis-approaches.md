...
The main episodes read EDM4eic data with **uproot** through an MCP server, because that needs
nothing heavier than the tools already in `eic-shell`. But uproot is one choice among many. Here
is the **same** Λ⁰ → p π⁻ analysis written four ways — each reproduces the same peak near
**1.1157 GeV**. Use whichever fits your stack.

Every example reads one of the dataset's `root://` files directly (no download), selects protons
(PDG `2212`) and π⁻ (PDG `-211`), builds proton–π⁻ pairs, and histograms the invariant mass. Full
scripts are in [`extras/`](https://github.com/aprozo/tutorial-mcp/tree/main/extras). In the
...
same peak. The physics does not care which library you use — choose for convenience.

:::::::::::::::::::::::::::::::::::::::::::::

## 1. Stand-alone uproot (no MCP)

Plain Python — no server, no ROOT install. This is the engine the MCP server runs internally.
...
The core is a four-vector build and a Cartesian pairing:
...
The classic ROOT event loop — explicit, readable, and the form most ePIC analysis code uses. The
reconstructed momentum branches are `float`, so the `TTreeReaderArray` must be templated on
`<float>`.
...
Reads the data as the model intends: PODIO deserialises each event into a `Frame`, and you iterate
over typed objects (`p.getPDG()`, `p.getMomentum()`).
...
Both find the same Λ⁰. That is why pairing an AI assistant with an uproot MCP server is such a
cheap on-ramp — real analysis without the heavy install.
